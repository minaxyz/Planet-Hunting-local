...
def transitImpactParameter(stellarRadius, planetaryRadius, orbitalPeriod, transitDuration):
    return (((((stellarRadius*SOLAR_RADIUS - planetaryRadius*EARTH_RADIUS)**2)-((semiMajorAxis1(orbitalPeriod, transitDuration)*AU*math.sin((transitDuration * math.pi)/(orbitalPeriod*86400)))**2))**0.5)/(stellarRadius*SOLAR_RADIUS))

# Calculates the semi major axis using the Star Mass, Orbital Period and Orbital Radius
def semiMajorAxis1(MASS, PERIOD):
    #returned in metres
...
def planetOrbitalInclination(starRadius, planetRadius, MASS, PERIOD, transitDuration):
    return math.acos((transitImpactParameter(starRadius,planetRadius, PERIOD, transitDuration)*starRadius*SOLAR_RADIUS)/(semiMajorAxis1(MASS, PERIOD)*AU))*(180/math.pi)
